...
-
  ...
  - [Задание](#задание)
  - [Указания к выполннению задания](#Указания-к-выполннению-задания)
...
- **Вариант 19.**  б) напечатать слова текста в алфавитном порядке.

- **Вариант 20.**  б) напечатать все слова, отличающиеся от последнего слова, и совпадающие с конечным отрезком алфавита (`z`, `yz`, `xyz` и т.д.).

- **Вариант 21.**  б) напечатать все слова, отличающиеся от последнего слова, и имеющие максимальную длину.

- **Вариант 22.**  б) напечатать все слова, отличающиеся от последнего слова, и не содержащие повторяющихся букв.

- **Вариант 23.**  б) напечатать все слова, отличающиеся от последнего слова, и в которые каждая буква входит не менее двух раз.

- **Вариант 24.**  б) напечатать все слова, отличающиеся от последнего слова, в которых гласные буквы (`a`, `e`, `i`, `o`, `u`, `y`) чередуются с согласными.

## Указания к выполннению задания

В ОС Windows используется функция getch()
Функция getch() возвращает очередной символ, считанный с консоли, но не выводит этот сим­вол на экран. (Прототип:
conio.h)

В стандарте Cи этого нет, поэтому используем termios (Linux).

Для корректной работы в OC Linux (OC Mac) необходимо подключить
...
а в функии `main()` вначале  устанавливаем еременные управления консолью
...
выключение эха и канонического ввода
...
где
- ICANON — отключает строковый режим (теперь читаем посимвольно);
...
```
, а перед завершением программы восстанавливаем исходный режим терминала
